...
def run(model, data):
    ...
    result = E_step_run(data, model, result['all_trans_softcounts'], result['all_emission_softcounts'],
    ...
    correct_emission_predictions = np.expand_dims(model["guesses"], axis=1) @ np.expand_dims(state_predictions[0, :],
        ...
        1 - model["slips"], axis=1) @ np.expand_dims(state_predictions[1, :], axis=0)
    ...
    flattened_predictions = np.take_along_axis(correct_emission_predictions,
                                               (data['data'] != 0).argmax(axis=0)[:, None].T, axis=0)
